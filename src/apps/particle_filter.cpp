...
struct Particle_t {
    ...
    Particle_t(float x_in, float y_in, float t_in, int64_t w_in) : 
...
void update_particle_weight(Particle_t& particle, DoublePoint& delta_r) {
    ...
    if (state.grid.isCellInGrid(laser_end_cell) || state.grid(laser_end_cell) < 63) {
        particle.weight -= -12;
...
void sensor_model_updater(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const maebot_laser_scan_t *scan, void *user)
{
    ...
    for(int i = 0; i < scan->num_ranges; ++i){
        ...
        rotate_matrix_z(&x, &y, odo_state.theta + elapsed_time * odo_state.v_theta);
